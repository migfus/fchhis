      ...
      <div class="relative flex h-16 justify-between">
        ...
        >
          <div class="flex flex-shrink-0 items-center">
            <img
              class="hidden h-10 w-auto sm:block sm:ml-5"
              src="http://127.0.0.1:8000/images/logo.png"
              alt="Future Care Helping Hands"
            />
            <!-- <object class="block h-10 w-auto lg:hidden" data="/images/logo.svg" alt="Your Company" /> -->
            <span class="ml-3 font-semibold lg:block hidden xs:block"
              >FCHHIS</span
            >
          </div>
          <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
            ...
            >
              Calendar
              <div
                class="relative inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-black bg-gradient-to-r from-primary-500 to-secondary-200 rounded-full -top-2 right-1"
              >
                {{ $event.countEvent }}
              </div>
            </RouterLink>
            ...
            <Popover
              class="block xl:hidden relative border-b-2 border-transparent px-1 pt-1 text-sm font-medium text-gray-500 hover:border-gray-300 hover:text-gray-700 mt-5"
              v-slot="{ open }"
            >
              <PopoverButton
                :class="[
              ...
              >
                ...
                />
                <div
                  class="absolute hidden md:inline-flex lg:hidden items-center justify-center w-6 h-6 text-xs font-bold text-black bg-gradient-to-r from-primary-500 to-amber-200 rounded-full -top-2 -right-3"
                >
                  {{ $event.countEvent }}
                </div>
              </PopoverButton>
              ...
              >
                <PopoverPanel
                  class="absolute left-1/2 z-10 mt-5 w-screen max-w-md -translate-x-1/2 transform"
                >
                  <div
                  ...
                  >
                    <div class="relative grid bg-white gap-4 p-4">
                      <RouterLink
                        :to="{ name: 'calendar' }"
                        class="lg:hidden -m-3 flex items-start rounded-lg p-3 hover:bg-gray-50"
                      >
                        <span class="text-base font-medium text-gray-900"
                          >Calendar</span
                        >
                        <div
                          class="relative inline-flex items-center justify-center w-7 h-6 text-xs font-bold text-black bg-gradient-to-r from-primary-500 to-amber-200 rounded-full -top-2 right-1"
                        >
                          {{ $event.countEvent }}
                        </div>
                      </RouterLink>
      ...
      <div class="space-y-1 pt-2 pb-4">
        ...
        >
          <DisclosureButton style="width: 100%; text-align: left">
            {{ row.name }}
            <div
              v-if="row.link.name == 'calendar'"
              class="relative inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-black bg-gradient-to-r from-primary-500 to-amber-200 rounded-full -top-2 right-1"
            >
              20
            </div>
          </DisclosureButton>
