...
<template>
  ...
  >
    <div class="bg-white px-4 py-5 shadow sm:rounded-xl sm:p-6 mb-2">
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
          ...
          <p class="mt-1 text-sm text-gray-500">
            This information will be the user's credentials
          </p>
        </div>
          ...
          <div class="grid grid-cols-6 gap-6">
            ...
            <div class="col-span-6 sm:col-span-3">
              <label
                ...
                >Birth Place (Province)</label
              >
              <select
                v-model="bday_province_id"
                id="bday_province"
                name="bday_province"
            ...
            <div class="col-span-6 sm:col-span-3">
              <AppSelect
                v-model="$user.params.address_id"
                placeholder="Birth Place (City)"
                name="bplace_city"
                :errors="errors"
              >
                <option v-if="!bday_province_id" value="1">
                  Select Province
                </option>
                <option
                  v-for="row in $address.content.find(
                    (item) => item.id === bday_province_id,
                  ).cities"
                  :key="row.id"
                  :value="row.id"
                >
            ...
            <div class="col-span-6 sm:col-span-3">
              <AppSelect
                ...
                :errors="errors"
              >
                <option v-if="!address_province_id" value="1">
                  Select Province
                </option>
                <option
                  v-for="row in $address.content.find(
                    (item) => item.id === address_province_id,
                  ).cities"
                  :key="row.id"
                  :value="row.id"
                >
            ...
            <div class="col-span-6">
              <AppInput
                v-model="$user.params.address"
                placeholder="Address (Specific)"
                name="address"
                :errors="errors"
              />
            </div>
      ...
      <div v-for="(row, index) in errors" :key="index" class="text-red-600">
        {{ `[${index.toUpperCase()}] = ${row}!` }}
      </div>
    </div>
  </Form>
