...
import AddAgentCard from '../forms/AddAgentCard.vue'
...
<template>
  <BasicTransition>
    <AddAgentCard v-if="$users.config.form == 'add'" />
  </BasicTransition>
  ...
  <div class="bg-white px-4 py-5 shadow sm:rounded-xl sm:p-6">
    <div class="grid grid-cols-6 gap-6">
      ...
      <div class="col-span-6 flex justify-end gap-1">
        <AppButton :loading="$users.config.loading" color="white"
          >Print</AppButton
        >
        <AppButton
          @click="$users.ChangeForm('add')"
          :loading="$users.config.loading"
          color="success"
          >Add</AppButton
        >
      </div>
