...
export const SumOfTransaction = (trans: Array<{ amount: number }>) => {
  if (trans)
    return NumberAddComma(
      trans.reduce((added, row) => added + Number(row.amount), 0),
    )
...
export const BalanceToPay = (
  trans: Array<{ amount: number }>,
  pay_type: { name: string },
  plan_detail: TGPlanDetail,
) => {
  if (!trans) return '0.00'

  let payment = Number(PlanToPay(pay_type, plan_detail))
  return NumberAddComma(
    Number(
      payment - trans.reduce((added, row) => added + Number(row.amount), 0),
    ),
  )
}

export const DueStatusColor = (dueDate: Date, target = 'text') => {
  if (moment().diff(dueDate, 'months') >= 1) return `${target}-red-500`
  else if (moment().diff(dueDate, 'days') >= 1) return `${target}-yellow-500`
  else return ''
}

export const DueStatusDesc = (dueDate: Date) => {
  if (!dueDate) return 'N/A'
  if (moment().diff(dueDate, 'months') >= 1) return `Overdue`
  else if (moment().diff(dueDate, 'days') >= 1) return 'Grace'
