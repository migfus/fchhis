...
export const ClientAllPrint = (content: TGAuthContent) => {
  ...
  let payment = NumberAddComma(
    PlanToPay(content.info.pay_type, content.info.plan_detail),
  )
  let balance = BalanceToPay(
    content.client_transactions,
    content.info.pay_type,
    content.info.plan_detail,
  )
  ...
  const printContent = [
    ...
    [
      ...
      {
        v: `Registerd At: ${moment(content.created_at).format('MMM D, YYYY hh:mm A')}`,
        ...contentStyle([1, 1, 1, 1]),
      },
      { v: ``, ...contentStyle([1, 1, 1, 1]) },
    ],
    [
      {
        v: `Sex: ${content.info.sex ? 'Male' : 'Female'}`,
        ...contentStyle([1, 1, 1, 1]),
      },
      { v: ``, ...contentStyle([1, 1, 1, 1]) },
      {
        v: `Mobile #: +63 ${content.info.cell}`,
        ...contentStyle([1, 1, 1, 1]),
      },
      { v: ``, ...contentStyle([1, 1, 1, 1]) },
    ],
    [
      {
        v: `Birth Day: ${moment(content.info.bday).format('MMM D, YYYY')}`,
        ...contentStyle([1, 1, 1, 1]),
      },
      { v: ``, ...contentStyle([1, 1, 1, 1]) },
      {
        v: `Birth Place: ${$address.CityIDToFullAddress(content.info.bplace_id)}`,
        ...contentStyle([1, 1, 1, 1]),
      },
      { v: ``, ...contentStyle([1, 1, 1, 1]) },
    ],
    [
      {
        v: `Address: ${content.info.address}, ${$address.CityIDToFullAddress(content.info.address_id)}`,
        ...contentStyle([1, 1, 1, 1]),
      },
      { v: ``, ...contentStyle([1, 1, 1, 1]) },
    ...
    [
      {
        v: `Due Date: ${content.info.due_at ? moment(content.info.due_at).format('MMM D, YYYY') : 'N/A'}`,
        ...contentStyle([1, 1, 1, 1]),
      },
      { v: ``, ...contentStyle([1, 1, 1, 1]) },
    ...
    ...content.beneficiaries.map((row) => {
      ...
      data.push({
        v: moment(row.bday).format('MMM D, YYYY'),
        ...contentStyle([1, 1, 1, 1]),
      })
      data.push({
        v: moment(row.created_at).format('MMM D, YYYY hh:mm A'),
        ...contentStyle([1, 1, 1, 1]),
      })
    ...
    ...content.client_transactions.map((row) => {
      ...
      data.push({
        v: `${row.plan_details.plan.name} (${row.pay_type.name})`,
        ...contentStyle([1, 1, 1, 1]),
      })
  ...
  XLSX.utils.book_append_sheet(
    wb,
    ws,
    content.name.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, ''),
  )
  XLSX.writeFile(
    wb,
    `${content.name}_${moment().format('YYYY-MM-DD_HH-mm-ss')}.xlsx`,
  )
}
